<template>
    <AppLayout title="Editar Projeto">
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar Projeto</h2>
        </template>
...
export default {
    ...
    mounted: function () {
        $("#form_projects #members").select2().val(route().current('projects.create') ? this.$page.props.auth.user.id : '').trigger('change');

        let startDate = this.project.start_date;
        let parts = startDate.split('/');
        startDate = `${parts[2]}-${parts[1]}-${parts[0]}`;

        let expectedCompletionDate = this.project.expected_completion_date;
        parts = expectedCompletionDate.split('/');
        expectedCompletionDate = `${parts[2]}-${parts[1]}-${parts[0]}`;

        this.form.title = this.project.title;
        this.form.description = this.project.description;
        this.form.start_date = startDate;
        this.form.expected_completion_date = expectedCompletionDate;

        let arrIdMembers = [];
        if (this.project.members.length) {
            this.project.members.map((member) => {
                arrIdMembers.push(member.user_id);
            });
            $("#form_projects #members").val(arrIdMembers).trigger('change');
        }
    }
};
